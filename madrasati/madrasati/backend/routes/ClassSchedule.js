...
router.post("/upload", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    for (const [index, row] of data.entries()) {
      ...
      // Validate teacherId
      logger.debug("Validating teacherId", {
        teacherId: row.teacherId,
        row: index + 2,
      });
      const teacher = await Teacher.findOne({ userid: row.teacherId });
      if (!teacher) {
        logger.error("Teacher not found for teacherId", {
          row: index + 2,
          teacherId: row.teacherId,
        });
        return res.status(404).json({
          error: `Teacher with userid ${row.teacherId} not found in row ${
            index + 2
          }`,
        });
      }
      ...
      schedules.push({
        classId,
        teacherId: row.teacherId, // Use the string userid
        subject: row.subject,
        dayOfWeek: row.dayOfWeek,
        startTime: row.startTime,
        endTime: row.endTime,
